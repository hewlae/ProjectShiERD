...
while date < end_date:
    ...
    reanal_file = reanalysis_dir+'/output00000000'
    reanal = read_out_file(reanal_file)
    reanal_data = pd.DataFrame()
    for wg,pp in wg_dic.items():
        reanal_value = reanal.get_part('node',pp).depth
        reanal_data[wg] = reanal_value   
    # Make each DataFrame     
    if analysis_date == start_date: 
        obsflow = obs_data
        reanalflow = reanal_data
    else:
        obsflow = pd.concat([obsflow, obs_data])
        reanalflow = pd.concat([reanalflow, reanal_data])
    date += datetime.timedelta(minutes=cycle)
    analysis_date = date.strftime('%Y%m%d%H%M')
print(obsflow.index)
print(reanalflow.index)
...
for wg,pp in wg_dic.items():
    ...
    sns.lineplot(x = reanalflow.index, y= wg, data = reanalflow, color="magenta", ax=ax1, linewidth=2)
    ax1.legend(labels=['simulation', 'observation', 'DAanalysis'], loc = 5, fontsize = '15')
